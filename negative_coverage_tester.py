...
import argparse
from bfs_full_edge import trace_to_str
...
def parse(graph, tokens):
    ...
    while len(queue) > 0:
        ...
        if len(token_stream) > 0:
            ...
            error_actions = graph.empty_table_entries[cur_node.label]
    ...
    return False, "epsilon",  graph.nodes[0].label
...
if __name__ == '__main__':
    ...
    command = "hyacc -v -g -O0 " + command_automaton_map[args[0]] + " " + args[1]
    os.system(command)

    graph = construct_graph(args)
    print(graph.empty_table_entries)
    #utils.remove_intermediary_files()
    ...
    for i in tqdm(range(len(tokenized))):
        ...
        if result[0]:
            ...
            state = result[2]
            token = result[1]
            errors[state].add(token)
    # calculate difference
    amount = 0
    for state in graph.empty_table_entries:
        uncovered = graph.empty_table_entries[state] - errors[state]
        amount += len(uncovered)
        for entry in uncovered:
            print("Empty entry  "+entry+" on state "+str(state)+" not covered")
    print("Total of "+str(amount)+ " empty entries not covered")
