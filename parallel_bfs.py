...
def bfs_delete_cover(S, E, T, graph):
    ...
    while True:
        ...
        func = partial(parallel_flood_delete, graph, seen_edges)
        pool = multiprocessing.Pool(multiprocessing.cpu_count() - 1)
        if len(E - seen_edges) == 0:
            return prefixes
        results = pool.map(func, cur_layer)
        pool.close()
        pool.join()
        merged_fresh = []
        merged_stale = []
        # merge current layers results
        for result in results:
            merged_fresh += result[0]
            merged_stale += result[1]
            # check if we have come globally unstuck
            if len(result[0]) > 0 and globaly_stuck:
                globaly_stuck = False
            # save stuck prefix if it got locally stuck
            if len(result[0]) == 0 and not globaly_stuck:
                prefixes.append(result[2])

        results = None
        next_layer = []
        # explore the next layer building on recently discovered edges
        for result in merged_fresh:
            if result is not None:
                seen_edges.add(result[0])
                next_layer.append(result[1])
        # if we have just seen the last batch of edges remember to add them
        if len(E - seen_edges) == 0:
            prefixes += next_layer
        # add to list of prefixes that repeat edges
        for result in merged_stale:
            if result is not None:
                stale_prefixes.append(result[1])

        # check if we are properly stuck
        if len(next_layer) == 0:
            next_layer = stale_prefixes
            stale_prefixes = []
            globaly_stuck = True
        print("remaining")
...
def parallel_flood_delete(graph, seen_edges, path):
    stale_edges = []
    fresh_edges = []
    cur_stack = path['stack']
    cur_trace = path['trace']
    ...
    # check for bad test case
    if len(cur_trace) > 2 * len(graph.edges):
        return fresh_edges, stale_edges, path
    ...
    if not path['modified'] and len(cur_trace) > 1 and (
        ...
        if prev_follow_set.isdisjoint(cur_follow_set) and not get_can_complete_without_terminal(prev_state, graph):
            ...
            path_copy['modified'] = True
            # add this extra path to the back of the queue and go on
            fresh_edges.append((edge_copy, path_copy))

    # just continue if we already modified an edge in this traversal
    if path['modified']:
        stale_edges.append((None, path.copy()))
        return fresh_edges, stale_edges, path
    ...
    if len(cur_trace) > 0 and cur_trace[-1].is_return:
        ...
        if edge in seen_edges:
            stale_edges.append((edge, new_path))
        else:
            fresh_edges.append((edge, new_path))
    else:
        # if the previous step wasn't a reduce then we should shift on terminals and check
        # for valid reductions on all edges
        for edge in cur_node.edges:
...
def bfs_cover(S, E, T, graph):
    ...
    while True:
        cur_layer = next_layer
        func = partial(parallel_flood, graph, seen_edges)
        pool = multiprocessing.Pool(multiprocessing.cpu_count() - 1)
        if len(E - seen_edges) == 0:
            return prefixes
        results = pool.map(func, cur_layer)
...
def parallel_flood(graph, seen_edges, path):
    stale_edges = []
    fresh_edges = []
    cur_stack = path['stack']
    cur_trace = path['trace']
    ...
    if len(cur_trace) > 0 and cur_trace[-1].is_return:
        ...
        if shift_edge in seen_edges:
            stale_edges.append((shift_edge, new_path))
        else:
            fresh_edges.append((shift_edge, new_path))
    else:
        # if the previous step wasn't a reduce then we should shift on terminals and check
        # for valid reductions on all edges
        for edge in cur_node.edges:
